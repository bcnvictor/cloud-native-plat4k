            ...
            return (
              <Card key={name} padding="none">
                ...
                <div className="grid grid-cols-3 gap-0 divide-x divide-border border-b border-border">
                  {[
                    { label: 'Nodes', value: m.nodesActive },
                    { label: 'CPU', value: `${m.cpuUsed}/${m.cpuTotal}` },
                    { label: 'RAM', value: `${m.ramUsedGi}/${m.ramTotalGi} Gi` },
                  ].map(({ label, value }) => (
                ...
                <div className="px-4 py-3">
                  ...
                  <ul className="flex flex-col gap-1.5">
                    {m.namespaces.map((ns) => (
                      <li key={ns.name} className="flex items-center gap-2">

from shared.models import sanitize_k8s_label_value

from backend.core.config import settings

_ALLOWED_FRAMEWORKS = {"python", "nodejs", "go", "generic"}

# Maps template/framework names to their CI framework key.
# Scaffolded apps store the template name as their framework (see app_service),
# so every template name must resolve to an allowed CI framework key here.
_FRAMEWORK_ALIASES = {
    "python-fastapi": "python",
    "python-flask": "python",
    "react-vite": "nodejs",
    "node-express": "nodejs",
}


def _yaml_escape(value: str) -> str:
    """Escape a string for insertion in a YAML double-quoted scalar."""
    return value.replace("\\", "\\\\").replace('"', '\\"').replace("\n", "\\n").replace("\r", "\\r")


def _ci_templates_project() -> str:
    if settings.GITLAB_CI_PROJECT:
        return settings.GITLAB_CI_PROJECT
    ns = settings.GITLAB_BOT_NAMESPACE or "cnp"
    return f"{ns}/cnp-ci-modules"

def generate_gitlab_ci(app_slug: str, app_id: int, framework: str, owner: str = "unknown", cluster_name: str = "aks") -> str:
    """Generate the minimal .gitlab-ci.yml to inject into a user repo."""
    framework = _FRAMEWORK_ALIASES.get(framework, framework)
    if framework not in _ALLOWED_FRAMEWORKS:
        raise ValueError(f"Unknown framework '{framework}' — allowed: {_ALLOWED_FRAMEWORKS}")

    templates_project = _ci_templates_project()
    safe_slug = _yaml_escape(app_slug)
    safe_owner = _yaml_escape(sanitize_k8s_label_value(owner))
    gitops_repo = _yaml_escape(settings.GITOPS_REPO_URL)
    safe_cluster = _yaml_escape(cluster_name)
    gitops_app_path = f"apps/{safe_cluster}/{safe_slug}"

    lines = [
        "# Auto-generated by CNP — do not edit manually\n",
        "variables:\n",
        f'  CNP_APP_NAME: "{safe_slug}"\n',
        f'  CNP_APP_ID: "{app_id}"\n',
        f'  CNP_APP_OWNER: "{safe_owner}"\n',
        f'  CNP_CLUSTER_NAME: "{safe_cluster}"\n',
        f'  CNP_GITOPS_REPO: "{gitops_repo}"\n',
        f'  CNP_GITOPS_APP_PATH: "{gitops_app_path}"\n',
        "\n",
        "include:\n",
        f"  - project: '{templates_project}'\n",
        "    ref: main\n",
        "    file: '/base/pipeline.yml'\n",
        f"  - project: '{templates_project}'\n",
        "    ref: main\n",
        "    file: '/security/gitleaks.yml'\n",
        f"  - project: '{templates_project}'\n",
        "    ref: main\n",
        "    file: '/security/trivy.yml'\n",
        f"  - project: '{templates_project}'\n",
        "    ref: main\n",
        "    file: '/security/hadolint.yml'\n",
    ]
    if framework != "generic":
        lines += [
            f"  - project: '{templates_project}'\n",
            "    ref: main\n",
            f"    file: '/frameworks/{framework}.yml'\n",
        ]
    return "".join(lines)

